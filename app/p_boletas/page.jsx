...
import { conversion, getCalificaciones, getCalificacionesAlumnos, getLugaresArea1, getMateriasGrupo, Imprimir } from '../utils/api/calificaciones/calificaciones';
...
function P_Boletas() {
    ...
    const Bus = async () => {
        ...
        dataMaterias.map((materia) => {
            for (let bi = 1; bi <= bimestre; bi++) {
                ...
                evaluaciones = 0
                let calificacion = 0
                const mat = dataCalificaciones.materias.find((mat) => mat.numero === materia.numero)
                if (mat) {
                    if (materia.area === 1 || materia.area === 4) {
                        if (mat.actividad === "no") {
                            const calif = dataCalificaciones.calificaciones.find((calif) =>
                            ...
                            const actividad = dataCalificaciones.actividades.filter((act) => act.materia === mat.numero)
                            actividad.map((act) => {
                                const califAct = dataCalificaciones.calificaciones.find((calAct) =>
        ...
        for (let i = 1; i <= bimestre; i++) {
            ...
            calificaciones.forEach((calificacion) => {
                if (calificacion.area === 1 && (calificacion.materia !== "LUGAR" && calificacion.materia !== "PROMEDIO")) {
            ...
            let promedioArea4 = contArea4 > 0 ? (sumaPromedioArea4 / contArea4).toFixed(1) : 0;
            if (promedioArea1 < 5.0) promedioArea1 = 5.0
            if (promedioArea4 < 5.0) promedioArea4 = 5.0
            let promedioExistenArea1 = calificaciones.find(cal => cal.materia === "PROMEDIO")
            ...
            const lugaraArea1 = lugaresArea1.find(lugar => lugar.numero === alumnosHorario.numero)
            let lugarExistenArea1 = calificaciones.find(cal => cal.materia === "LUGAR")
            ...
            const lugaraArea4 = lugaresArea4.find(lugar => lugar.numero === alumnosHorario.numero)
            let lugarExistenArea4 = calificaciones.find(cal => cal.materia === "PLACE")
            ...
            let promedioExistenArea4 = calificaciones.find(cal => cal.materia === "AVERAGE")
        ...
        if (checkPromedio) {
            ...
            calificaciones = calificaciones.map((materia) => {
                ...
                if ((materia.materia !== "LUGAR") && (materia.materia !== "PLACE")) {
                    ...
                    const promedio = evaluaciones > 0 ? (sumatoria / evaluaciones).toFixed(1) : 0;
                    return {
                        ...materia,
                        promedio: promedio
                    };
        ...
        if (checkCaliLetra) {
            ...
            calificaciones.map((calificacion) => {
                ...
                if (calificacion.materia === "PLACE") { return calificacion; }
                ...
                if (checkPromedio) calificacion.promedio = conversion(parseFloat(calificacion.promedio), tipo);
                return calificacion;
            });
        }
        setisLoading(false);
    ...
    const handleVerClick = () => {
        ...
        const Enca1 = (doc) => {
            if (!doc.tiene_encabezado) {
                ...
                doc.nextRow(12);
                doc.nextRow(4);
                doc.ImpPosX("AV.SANTA ANA N° 368 COL. SAN FRANCISCO CULCHUACÁN C.P. 04420", 50, doc.tw_ren, 0, "L");
                doc.nextRow(4);
                doc.ImpPosX("ACUERDO N° 09980051 DEL  13 AGOSTO DE 1998", 65, doc.tw_ren, 0, "L");
                doc.nextRow(4);
                ...
                doc.nextRow(4);
                doc.ImpPosX("El acercamiento al colegio será una forma para asegurar el éxito del alumno", 50, doc.tw_ren, 0, "L");
                doc.nextRow(4);
                doc.printLineV();
        ...
        Enca1(reporte)
        reporte.nextRow(10);
        reporte.ImpPosX(`ALUMNO ${alumnosHorario.nombre || ""}`, 15, reporte.tw_ren, 0, "L")
        reporte.nextRow(4)
        reporte.ImpPosX(`GRUPO: ${grupo.horario || ""}`, 15, reporte.tw_ren, 0, "L")
        if (selectedOption === "español") {
            reporte.ImpPosX(`ESPAÑOL`, 90, reporte.tw_ren, 0, "L");
            const data = body
                .filter((c) => c.area <= 3)
                .map((boleta) => [
                    { content: boleta.materia.toString(), styles: { fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? '12' : '10' } },
                    { content: boleta.EB1?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? '12' : '10' } },
                    { content: boleta.EB2?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? '12' : '10' } },
                    { content: boleta.EB3?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? '12' : '10' } },
                    { content: boleta.EB4?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? '12' : '10' } },
                    { content: boleta.EB5?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? '12' : '10' } },
                    { content: boleta.promedio?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PROMEDIO" ? '12' : '10' } },
                ]);
            ...
            const data = body
                .filter((c) => c.area >= 3)
                .map((boleta) => [
                    { content: boleta.materia.toString(), styles: { fontStyle: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                    { content: boleta.EB1?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                    { content: boleta.EB2?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                    { content: boleta.EB3?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                    { content: boleta.EB4?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                    { content: boleta.EB5?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                    { content: boleta.promedio?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "PLACE" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                ]);
            reporte.generateTable(header, data)
        } else {
            reporte.ImpPosX(`ESPAÑOL`, 90, reporte.tw_ren, 0, "L");
        ...
        reporte.nextRow(6);
        reporte.ImpPosX("NOMBRE Y FIRMA DEL PADRE O TUTOR", 70, reporte.tw_ren, 0, "L");
    ...
    const PDF = async () => {
            ...
            dataMaterias.map((materia) => {
                for (let bi = 1; bi <= bimestre; bi++) {
                    ...
                    if (mat) {
                        ...
                        let materiaExistente = calificaciones.find(cal => cal.materia === materia.descripcion && cal.alumnos === alumno.numero);
                        if (materiaExistente) {
                            materiaExistente[`EB${bi}`] = parseFloat(calificacion.toFixed(1));
                        } else {
                            calificaciones.push({
        ...
        for (let i = 1; i <= bimestre; i++) {
            ...
            alumnos.map((alumno) => {
                ...
                sumaPromedioArea4 = 0
                contArea4 = 0
                let calificacionesArea1 = calificaciones.filter(alum => alum.alumnos === alumno.numero && alum.area === 1)
                calificacionesArea1.map((calificacion) => {
                    if (calificacion.area === 1 && (calificacion.materia !== "LUGAR" && calificacion.materia !== "PROMEDIO")) {
                        sumaPromedioArea1 += calificacion[`EB${i}`]
                        contArea1++
                        if (indexArea1 === -1) {
                            indexArea1 = calificaciones.findIndex((p) => p.area === 2 && p.alumnos === alumno.numero)
                            if (indexArea1 === -1) {
                ...
                let promedioArea1 = contArea1 > 0 ? (sumaPromedioArea1 / contArea1).toFixed(1) : 0;
                if (promedioArea1 < 5.0) promedioArea1 = 5.0
                let promedioExistenArea1 = calificaciones.find(cal => cal.materia === "PROMEDIO" && cal.alumnos === alumno.numero)
                if (promedioExistenArea1) {
                    promedioExistenArea1[`EB${i}`] = promedioArea1
                } else {
                    calificaciones.splice(indexArea1, 0, {
                ...
                const lugaraArea1 = lugaresArea1.find(lugar => lugar.numero === alumno.numero)
                let lugarExistenArea1 = calificaciones.find(cal => cal.materia === "LUGAR" && cal.alumnos === alumno.numero)
                if (lugarExistenArea1) {
                    lugarExistenArea1[`EB${i}`] = lugaraArea1[i]
                } else {
                    calificaciones.splice(indexArea1 + 1, 0, {
                ...
                calificacionesArea4.map((calificacion) => {
                    if (calificacion.area === 4 && (calificacion.materia !== "PLACE" && calificacion.materia !== "AVERAGE")) {
                        ...
                        contArea4++
                        if (indexArea4 === -1) {
                            indexArea4 = calificaciones.findIndex((p) => p.area === 5 && p.alumnos === alumno.numero)
                            if (indexArea4 === -1) {
                                indexArea4 = calificaciones.findIndex((p) => p.area === 2 && p.alumnos === alumno.numero) + 2
                            }
                        }
                    }
                })
                let promedioArea4 = contArea4 > 0 ? (sumaPromedioArea4 / contArea4).toFixed(1) : 0;
                if (promedioArea4 < 5.0) promedioArea4 = 5.0
                let promedioExistenArea4 = calificaciones.find(cal => cal.materia === "AVERAGE" && cal.alumnos === alumno.numero)
                if (promedioExistenArea4) {
                    promedioExistenArea4[`EB${i}`] = promedioArea4
                } else {
                    calificaciones.splice(indexArea4, 0, {
                ...
                const lugaraArea4 = lugaresArea4.find(lugar => lugar.numero === alumno.numero)
                let lugarExistenArea4 = calificaciones.find(cal => cal.materia === "PLACE" && cal.alumnos === alumno.numero)
                if (lugarExistenArea4) {
                    lugarExistenArea4[`EB${i}`] = lugaraArea4[i]
                } else {
                    calificaciones.splice(indexArea4 + 1, 0, {
            ...
            if (alumAnt !== alumAct) {
                ...
                if (selectedOption === "español" || selectedOption === "ingles") {
                    const data = body
                        ...
                        .map((boleta) => [
                            { content: boleta.materia.toString(), styles: { fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                            { content: boleta.EB1?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                            { content: boleta.EB2?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                            { content: boleta.EB3?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                            { content: boleta.EB4?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                            { content: boleta.EB5?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                            { content: boleta.promedio?.toString() ?? "", styles: { halign: 'right', fontStyle: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? 'bold' : 'normal', fontSize: boleta.materia === "LUGAR" || boleta.materia === "PLACE" || boleta.materia === "PROMEDIO" || boleta.materia === "AVERAGE" ? '12' : '10' } },
                        ]);
                    newPDF.generateTable(header, data)
                }
    ...
    const PDFAlumno = () => {
        if (caliAlum) {
            ...
            configuracion = { ...configuracion, checkBoletaK: checkBoletaKinder, cicloFechas: cicloFechas, alumno: alumnosHorario.nombre, grupo: grupo.horario, selectedOption: selectedOption }
