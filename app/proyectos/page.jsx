    ...
    const infoFiltrada = BDRef.current.filter((proyecto) => {
      ...
      const coincideHost = host
        ? proyecto["host"]
            .toString()
            .toLowerCase()
            .includes(host.toLocaleLowerCase())
        : true;
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    if (res.status) {
      if (accion === "Alta") {
        ...
        setBasesDeDatosFiltrados([...basesDeDatosFiltrados, nuevoproyecto]);
      }
      if (accion === "Editar") {
        const index = basesDeDatosS.findIndex(
          (proyecto) => proyecto.id === data.id
        );
        if (index !== -1) {
