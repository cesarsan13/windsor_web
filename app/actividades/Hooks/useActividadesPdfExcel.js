...
export const useActividadesPdfExcel = (
  ...
  const handleVerClick = () => {
    ...
    const alignsIndex = [0];
    const tablaExcel = [["Asignatura", "Actividad"]];
    Enca1(reporte);
      ...
      reader.onload = (event) => {
        ...
        const convertedData = jsonData.map((item) => {
          return {
            ...
            EB1: isNaN(parseInt(item.EB1)) ? 0 : parseInt(item.EB1),
            EB2: isNaN(parseInt(item.EB2)) ? 0 : parseInt(item.EB2),
            EB3: isNaN(parseInt(item.EB3)) ? 0 : parseInt(item.EB3),
            EB4: isNaN(parseInt(item.EB4)) ? 0 : parseInt(item.EB4),
            EB5: isNaN(parseInt(item.EB5)) ? 0 : parseInt(item.EB5),
            baja: validateString(
              MAX_LENGTHS,
              "baja",
