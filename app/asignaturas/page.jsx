...
function Asignaturas() {
    ...
    const Buscar = () => {
        ...
        const infoFiltrada = asignaturas.filter((asignatura) => {
          const coincideId = tb_id ? asignatura["numero"].toString().includes(tb_id) : true;
    ...
    const onSubmitModal = handleSubmit(async (data) => {
      ...
      if (res.status) {
        ...
        if (accion === "Eliminar" || accion === "Editar") {
          ...
          if (index !== -1) {
            if (accion === "Eliminar") {
              const aFiltrados = asignaturas.filter((p) => p.numero !== data.numero);
    ...
    const handleVerClick = () => {
      ...
      cerrarModalVista();
        ...
        asignaturasFiltrados.forEach((asignatura) => {
          ...
          reporte.ImpPosX(asignatura.descripcion.toString(), 28, reporte.tw_ren, 25, "L");
          Enca1(reporte);
