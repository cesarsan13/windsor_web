...
import Acciones from "@/app/catalogos/actividades/components/Acciones";
import Busqueda from "@/app/catalogos/actividades/components/Busqueda";
import TablaActividades from "@/app/catalogos/actividades/components/tablaActividades";
...
import ModalActividades from "@/app/catalogos/actividades/components/ModalActividades";
import { confirmSwal, showSwal } from "@/app/utils/alerts";
import ModalVistaPreviaActividades from "@/app/catalogos/actividades/components/ModalVistaPreviaActividades";
    ...
    const infoFiltrada = actividades.filter((actividad) => {
      const coincideId = tb_id
        ? actividad["materia"]
            .toString()
            .toLowerCase()
            .includes(tb_id.toLowerCase())
        : true;
      const coincideDescripcion = td_desc
        ? actividad["materia"]
            .toString()
            .toLowerCase()
            .includes(tb_id.toLowerCase())
    ...
    if (res.status) {
      ...
      if (accion === "Eliminar" || accion === "Editar") {
        const index = actividades.findIndex(
          (fp) => fp.materia === data.materia
        );
        if (index !== -1) {
          if (accion === "Eliminar") {
            const fpFiltrados = actividades.filter(
              (fp) => fp.materia !== data.materia
            );
            ...
            if (bajas) {
              ...
              const fpActualizadas = actividades.map((fp) =>
                fp.materia === currentID && fp.secuencia === data.secuencia
                  ? { ...fp, ...data }
                  : fp
              );
        ...
        <div className="flex flex-col justify-start p-3">
          <div className="flex flex-wrap md:flex-nowrap items-start md:items-center">
            <div className="order-2 md:order-1 flex justify-around w-full md:w-auto md:justify-start mb-0 md:mb-0">
              <Acciones Alta={Alta} Ver={handleVerClick} />
            </div>
