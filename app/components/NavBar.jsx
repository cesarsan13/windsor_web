...
function NavBar() {
  ...
  useEffect(() => {
    const temaInicial = window.matchMedia("(prefers-color-scheme: dark)").matches ? false : true;
    setTema(temaInicial);

    const themeInicial = window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";
  ...
  return (
    <div className="navbar sticky top-0 h-full max-h-7 bg-slate-100 dark:bg-slate-700 z-[2]">
      <div className="navbar-start">
        ...
        <Link className="btn btn-ghost text-xl text-black dark:text-white" href="/">
          Sistema Escolar
        </Link>
      </div>
