...
function AlumnosPorClase() {
    ...
    useEffect(()=> {
        if(status === "loading" || !session) {
          ...
          const fetchData = async () => {
            ...
            const data = await getreportAlumn(token, bajas, selectedOption, alumno1V,alumno2V);
    ...
    const handleVerClick = async () => {
        if (alumnos1.id === undefined){
            showSwal("Oppss!", "Para imprimir, mínimo debe estar seleccionado un Alumno de 'Inicio'", "error");
          } else {
        const configuracion = {
            Encabezado: {
                Nombre_Aplicacion: "Sistema de Control Escolar",
