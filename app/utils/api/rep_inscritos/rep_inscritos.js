...
export const verImprimir = async (configuracion) => {
  ...
  bodyAlumnos.forEach((alumno) => {
    ...
    const detalleEncontrado = bodyDetalles.filter(
      (detalle) =>
    ...
    if (detalleEncontrado.length > 0) {
      detalleEncontrado.forEach((detalle) => {
        const productoEncontrado = bodyProductos.filter(
          (producto) =>
            producto.ref === "INS" &&
            String(producto.numero) === String(detalle.articulo)
        );
      ...
      if (si_suma) {
        ...
        if (horarioEncontrado) {
          newPDF.ImpPosX(
            horarioEncontrado.horario.toString() || "",
            125,
            newPDF.tw_ren,
            0,
            "L"
        ...
        newPDF.ImpPosX(
          formatNumber(det_inscripcion) || "0.00",
          195,
          newPDF.tw_ren,
          0,
          "R"
  ...
  newPDF.nextRow(4);
  newPDF.ImpPosX(
    `Total: ${formatNumber(total_inscripcion)}` || "0.00",
    195,
    newPDF.tw_ren,
    0,
    "R"
  ...
  newPDF.nextRow(4);
  newPDF.ImpPosX(
    `Total Alumnos: ${alumnos.toString() || "0"}`,
    195,
    newPDF.tw_ren,
    0,
    "R"
...
export const ImprimirExcel = (configuracion) => {
  ...
  bodyAlumnos.forEach((alumno) => {
    ...
    if (detalleEncontrado.length > 0) {
      ...
      if (si_suma) {
        const nombreCompleto = `${alumno.a_nombre || ""} ${
          alumno.a_paterno || ""
        } ${alumno.a_materno || ""}`.substring(0, 50);
        newBody.push({
          numero: alumno.numero,
          nombre: nombreCompleto,
          horario:
            bodyHorarios.find((horario) => horario.numero === alumno.horario_1)
              ?.horario || "",
