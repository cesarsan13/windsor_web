...
export const Imprimir = async (
    ...
    for (const dato of data) {
      if (dato.horario_1 > 0) {
        ...
        if (grupo_ant !== grupo_act) {
         ...
         const res = await fetch(`${process.env.DOMAIN_API}api/documentoscobranza/grupo`, {
          method: "PUT",
          body: JSON.stringify({
            alumno: dato.id,
  ...
  documentos.forEach((doc, index) => {
    ...
    if (si_Imp === true) {
      ...
      saldoTotal += saldo;
      total_General += saldo;
      newPDF.ImpPosX(saldo.toString(), 228, newPDF.tw_ren);
      ...
      if (isLastRecordForAlumno) {
        newPDF.ImpPosX(saldoTotal.toString(), 248, newPDF.tw_ren);
